# spec.md — livespec-impl-beads

This is the natural-language specification for `livespec-impl-beads`,
...
`livespec-impl-beads` is one realization of the abstract
...
through the pinned `bd` (beads) CLI in server mode. Work-items and
memos are beads issues in that tenant DB; the plugin never embeds a
...
DB. Everything above the store boundary — the 10-skill surface, the
...
the plugin. Everything else — the names of the ten skills, the
...
`livespec-impl-beads.compat` block.
...
**Beads issue (memo)** — One row in the same tenant DB carrying the
label `kind:memo`. Schema realization is defined in `contracts.md`
§"Memo beads-issue mapping". Memos are ordinary beads issues
distinguished only by the `kind:memo` label; `list-memos` filters on
it.
...
beads `prefix` equals the tenant DB name — the load-bearing identity
rule (so issue ids read back as `<prefix>-<suffix>`).
...
**Materialized view** — The current state of a work-item or memo, read
...
realization", `livespec-impl-beads` realizes the upstream-mandated
...
- The tenant DB lives on the shared `dolt sql-server`; connection
  ...
  under the `livespec-impl-beads` section (`connection` block). The
...
- The plugin connects in `bd` server mode via a FLAGS connection (per
  ...
  --non-interactive --quiet`. The server is externally managed
...
- The audit trail is the tenant DB's own version history plus the
  ...
  tree. Work-item / memo state is NOT git-tracked as files in the
  consumer repo (that is the plaintext sibling's model); it lives in the
  tenant DB.
- The materialized view of any work-item or memo is read back from `bd`
