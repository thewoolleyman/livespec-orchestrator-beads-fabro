...
3. `SPECIFICATION/scenarios.md`, the capture-impl-gaps flow line: replace "And the intake Definition-of-Ready routes it (a DoR-passing item lands `pending-approval`, approved into `ready` when its effective admission_policy is `auto`)" with "And the intake Definition-of-Ready routes it (a DoR-passing item transits `pending-approval` and proceeds to `ready`; admission policy is exercised at the admission valve, not by this routing)".
