...
`run` executes only a selected action. For a selected impl dispatch
action it invokes the existing
...
**Human valve actions.** `run` additionally accepts the three
human-valve action ids — `approve:<work-item-id>` (explicitly admits an
effective-`manual` `ready` item: sets `assignee` and transitions it to
`active`, subject to the per-repo WIP cap), `accept:<work-item-id>` (the
human leg of post-merge acceptance: `acceptance → done`), and
`reject:<work-item-id>:rework` / `reject:<work-item-id>:regroom`
(`acceptance → active` fix-forward; `acceptance → backlog` with the
merged change reverted). These are human-TRIGGERED valve commands, not
machine-path dispositions: the explicit action selection is the consent
(an up-front operation decision per §"Store-write consent discipline"),
each writes through the same store seam, and the journal records the
actor. This is the published surface the console invokes for the two
human valves (§"Dispatcher admission, WIP cap, and post-merge
acceptance"); the console never writes the ledger directly. The
valve-action behavior is exercised by `scenarios.md` Scenario 31.
...
`acceptance_policy`), `reject` routing out of `acceptance`, and the
non-convergence `backlog` bounce. These writes are EXEMPT from
...
Dispatcher's module docstring documents this boundary. The
human-triggered valve commands (`orchestrate run`
`approve:`/`accept:`/`reject:` action ids, per §"`orchestrate`") are
NOT machine-path dispositions — their consent is the operator's
...
1. **Capture / intake (augmented).** Work is filed as today via
   ...
   item lands in `pending-approval` (approved on into `ready` when its
   effective `admission_policy` is `auto`); an epic-shaped item lands in
...
3. **Dispatch (unattended, exceptions only).** The Dispatcher drains
   ...
   the maintainer in ONLY to approve an admission-held item (effective
   `admission_policy` `manual`, the spec-change / risky tier) or to
...
The Dispatcher MUST hold at the admission valve any item whose
effective `admission_policy` is `manual` (the first-class realization
of the prior `human-gated` spec-change marker) — it surfaces the held
item for the maintainer instead of dispatching it (the authoritative
valve contract is §"Dispatcher admission, WIP cap, and post-merge
acceptance"). On
...
`admission_policy == manual` (held at the admission valve until a human
explicitly admits it), and the prior `needs-regroom` disposition is the
lifecycle `bounce` back to the `backlog` state (re-decomposition). The
authoritative valve contract is §"Dispatcher admission, WIP cap, and
post-merge acceptance"; Scenarios 9–11 express the same vocabulary.
...
behaviors (hold manual-admission items; bounce on non-convergence; emit
...
The `capture-work-item` and `capture-impl-gaps` capture front-ends MUST run the intake Definition-of-Ready checklist over the six gates at capture and MUST route the resulting item into its lifecycle state accordingly — a single-coherent-done, autonomously-verifiable, autonomy-tiered, dependency-linked, repo-targeted, above-floor item lands in `pending-approval` (approved on into `ready` when its effective `admission_policy` is `auto`); an item with more than one coherent "done" (an epic) MUST land in `backlog` for decomposition; an item whose acceptance is not autonomously verifiable MUST land in `blocked` with `blocked_reason: needs-human`; an item with unresolved blockers is filed with its dependency edges linked (deriving the `blocked:dependency` lane) and MUST NOT land directly in `ready`.
...
An item MUST enter `backlog` on an intake Definition-of-Ready epic failure and MUST enter `backlog` on a Dispatcher non-convergence bounce; groom approval MUST transition the `backlog` item out by filing `ready` slices (the original item is regroomed-out, never silently dropped).
...
thread MAY be research-only. There is NO root `research/`
...
middle of the lifecycle — **admission** (`ready → active`) and
**acceptance** (`acceptance → done`). The **Dispatcher
(`dispatcher.py` `dispatch`/`loop`) is the sole enforcer** of both valves
and the WIP cap; the console only commands (a human triggers `approve`
for a manual item, through the `orchestrate` human-valve actions) and
observes — it never enforces. This realizes the
repo-agnostic two-valve pattern for the Beads/Fabro Dispatcher; the
state machine + the valve semantics are the cross-repo design of record.

### Admission valve (`ready → active`)

The valve's conditions are permission, capacity, and a resolvable
assignee:

- **Permission** is governed by the item's effective `admission_policy`
  (`item.admission_policy`, with `None` inheriting the safe default
  `manual`): an `auto` item is auto-admittable the moment it is `ready`;
  a `manual` item is HELD at the valve and surfaced for the maintainer
  on every pass (independent of capacity), and only an explicit human
  admission command admits it. The human's explicit admission IS the
  approval act — approval is the synchronous human-triggered admit
  (via the operator valve surface, §"`orchestrate`" → human valve
  actions), not a stored field; no approval evidence is persisted. The
  `admission_policy` field is the first-class realization that
  **replaces the prior `host-only` / `human-gated` text markers** —
  risky / irreversible work is held HERE, at admission, never by a
...
mapping") admission-eligible `ready` item (eligible = effective
`admission_policy` is `auto` AND dependencies clear AND an
assignee is resolvable), set its `assignee` (the reused field, not a new
`owner`), and transition it to `active`. The Dispatcher MUST NOT
auto-admit an item whose effective `admission_policy` is `manual`; it
MUST hold and surface it until a human explicitly admits it.
...
- **`accept` (`acceptance → done`)** is a **post-ship confirmation**
  ...
  - `human-only` — a human accepts from the console (via the
    `orchestrate run` `accept:<id>` valve action).
...
**admission + reversibility**, not a pre-merge acceptance hold. The AI
...
exercised by `scenarios.md` (the WIP-capped top-ranked admission, the
manual-admission hold, the complete-merges-on-green, and the
...
collapses the two human-delegable valves of §"Dispatcher admission, WIP
cap, and post-merge acceptance" to their autonomous leg for the duration
...
- treat every item's effective `admission_policy` as `auto` — admitting
  even items whose stored `admission_policy` is `manual`, so no item is
  HELD at the admission valve for a human (overriding, for this run only,
  the "MUST hold and surface it until a human explicitly admits it" rule
  of §"Admission valve (`ready → active`)");
...
- **Opt-in flag.** The run path MUST require an explicit opt-in:
  `orchestrate run --mode autonomous` (alongside the existing
...
telemetry), carrying at minimum the work-item id, which valve was
collapsed (`admission` / `acceptance` / `needs-human`), and what the LLM
...
- Under full autonomous mode the Dispatcher MUST admit an
  otherwise-eligible `manual`-admission item without a human approval.
...
> issue has a real, non-sentinel `rank`. These are enforced by this
