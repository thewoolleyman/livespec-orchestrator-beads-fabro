...
- **`dispatcher.answer_disposition`** (enum `human` | `consensus`, default
  **`human`**) — who may answer an **attention item**: a work-item resting at
  ...
  This setting adds NO new `drive` valve — the twelve human valve actions of
  §"`drive`" remain twelve — because the per-item override is a label, not a
  runtime valve press.
...
attribute the factory emits. Each class is Codex-backed ONLY when the dispatch
