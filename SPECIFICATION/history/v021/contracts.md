...
Under the deterministic lifecycle state machine, this behavior is
re-expressed in the lifecycle vocabulary (the authoritative new contract
is §"Dispatcher admission, WIP cap, and post-merge acceptance"): the
`human-gated` marker is realized by the item's effective
`admission_policy == manual` (the Dispatcher refuses to ADMIT it until it
is explicitly approved into `ready`), and the non-convergence
`needs-regroom` disposition is the lifecycle `bounce` back to the
`backlog` state (re-decomposition). The implement slice that lands the
state machine consolidates the `human-gated` / `needs-regroom` scenarios
(`scenarios.md` Scenarios 9–11) into that vocabulary; until then those
scenarios remain valid descriptions of the currently-shipped behavior.
...
for a manual item) and observes — it never enforces. This realizes the
...
(approval ≡ `ready` membership — entering `ready` IS approving). So the
valve's remaining conditions are permission, capacity, and a resolvable
assignee:

- **Permission** was settled upstream at the `pending-approval → ready`
  (`approve`) transition, governed by the item's effective
  `admission_policy`: `auto` auto-approves once at groom time; `manual`
  (the default, via inherit) waits for a human's explicit `approve`. The
...
The Dispatcher MUST, when a WIP slot frees, admit the **highest-`rank`**
admission-eligible `ready` item (eligible = dependencies clear AND an
assignee is resolvable), set its `assignee` (the reused field, not a new
`owner`), and transition it to `active`. The Dispatcher MUST NOT admit
an item whose effective `admission_policy` is `manual` until it has been
explicitly approved into `ready`.
...
exercised by `scenarios.md` (the WIP-capped highest-rank admission, the
...
`livespec/dev-tooling/implementation/research/beads-schema-mapping.md`
§Part 1. The logical work-item field set tracks the shared
...
- `rank` — beads `metadata.rank` (a structured value carried in the
  ...
  `str`. A legacy beads issue whose `metadata` lacks `rank` reads back
  through the shared bottom-sentinel
