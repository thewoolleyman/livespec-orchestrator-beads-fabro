...
orchestrator's persistent work-items / memos store performed on the
...
`livespec/SPECIFICATION/contracts.md` §"Interactive dialogue
ownership (orchestrator-side)", the consent dialogue is
orchestrator-owned: this plugin's six heavyweight SKILL.md
front-ends — `capture-impl-gaps`, `capture-memo`,
`capture-spec-drift`, `capture-work-item`, `implement`,
`process-memos` — are exactly those front-ends and are the governed
surface of this discipline.
...
- **Explicit confirmation** — the skill presents the assembled
  record (or the disposition about to be recorded) and the user
  confirms it. Examples: `capture-work-item`'s "file?" step,
  `capture-impl-gaps`' per-gap confirm, `process-memos`' per-memo
  disposition choice.
- **Consent-by-authorship** — the user-supplied free text fully
  determines the single resulting write and the skill performs no
  other store write. Example: `capture-memo`'s deposit — the typed
  memo text IS the consent for the single append; no second
  confirmation prompt is required (low-friction deposit is the
  point).
...
operation class (e.g. "auto-dispose all pending memos as discard";
"file every detected gap without asking"). A waiver MUST be
...
create net-new work-items or memos on its own initiative. The
Dispatcher's module docstring documents this boundary.

### Out-of-scope surfaces

The four thin-transport skills (`list-memos`, `list-work-items`,
`next`, `detect-impl-gaps`) are query-only by contract (per
...
This subsection restates the realization's fundamental behaviors as
explicit normative clauses so the mechanical gap-detector and the
heading-coverage map can hold the impl accountable; the surrounding
prose subsections remain as augmentation. Where behaviors 1-3, 7, and 8
were previously stated only as prose in §"The four maintainer
touchpoints" / §"Calibration telemetry and the single Fabro tweak",
that prose stays in place as augmentation but the authoritative
normative statement is now the clause line here.

The `capture-work-item`, `capture-impl-gaps`, and `process-memos` (impl-bound) capture front-ends MUST run the intake Definition-of-Ready checklist over the six gates at capture and MUST tag the resulting item `ready`, `needs-regroom`, or `not-yet-actionable` accordingly — a single-coherent-done, autonomously-verifiable, autonomy-tiered, dependency-linked, repo-targeted, above-floor item is tagged `ready`; an item with more than one coherent "done" (an epic) MUST be tagged `needs-regroom`; an item whose acceptance is not autonomously verifiable, or that has unresolved blockers, MUST be tagged `not-yet-actionable` and MUST NOT be filed as `ready`.
...
The Dispatcher MUST refuse to auto-dispatch a `human-gated` (spec-change) item — it surfaces it for the maintainer instead.

On factory NON-CONVERGENCE (a dispatched slice that will not converge through the janitor gate) the Dispatcher MUST mark the item `needs-regroom` and SURFACE it (escalate-don't-drop), never infinite-retry.

The Dispatcher MUST emit calibration telemetry: an outcome signal plus mechanical size proxies recorded on the EXISTING Dispatcher journal (the journal → Honeycomb leg already designed in the operability preconditions), with NO new always-on service.
