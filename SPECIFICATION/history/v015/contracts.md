...
implementations architecture"). The five heavyweight ops are
`capture-impl-gaps`, `capture-spec-drift`, `capture-work-item`,
`implement`, and `groom`. `groom` is the FIFTH heavyweight op; it is
ALSO the one new maintainer front-end catalogued under §"Skills —
...
that section for its read-only-draft / human-approves contract. The four
non-groom ops are detailed in the subsections that follow.
...
orchestrator-owned: this plugin's five heavyweight front-ends —
`capture-impl-gaps`, `capture-spec-drift`, `capture-work-item`,
`implement`, and `groom` — are exactly those front-ends and are the
governed surface of this discipline. (`groom` is a consented
...
slice-size calibration".) Each front-end's consent flow lives in the
shared `.claude-plugin/prose/<op>.md` artifact the per-runtime SKILL.md
bindings read (per §"Heavyweight authored skills (5)").
...
authored skill (the fifth, per §"Heavyweight authored skills (5)"), so
