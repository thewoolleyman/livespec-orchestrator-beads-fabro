...
gradeable-assertion count, and name both remedies: author gradeable criteria by
groom or edit, or deliberately select `human-only` where machine grading is
genuinely inapplicable. Its handoff MUST be a non-dispatch inspection or repair
handoff and MUST NOT carry `impl:<work-item-id>` or any other action the
Dispatcher would reject. A `human-only` item and an item whose effective
workflow variant is groom-kind MUST NOT produce this fact. The fact clears when
the item gains gradeable criteria, changes to `human-only`, selects an exempt
groom-kind variant, or leaves `ready`. The fact and every candidate-producing
...
gradeable assertions; `human-only` is eligible with respect to this decision
because the human, not the AI pass, owns its grading. The pre-dispatch wall,
...
item can be repaired when it is next touched.
