...
- **Gate coverage — which runs are gated at all.** The Dispatcher observes a per-run cost signal ONLY for a dispatched run that BOTH reached a successful terminal outcome AND carries a confirmed run record (a run id resolvable against the cost source). Two classes of run are therefore NOT gated:
...
- **Verdict keying.** For a GATED run whose cost signal is **UNOBSERVABLE** (no cost is readable for the run), the verdict is keyed on **whether the invocation named an `--item`** — the contract's proxy for whether a human is present:
...
- **Gate coverage is contracted AS-IS and is knowingly narrower than a fail-closed ideal.** The shipped gate covers only successful runs with a resolvable run id, and skips FAIL-OPEN on an unresolvable one. A.3 names both gaps in the ratified text rather than hiding them, because the design record locks only the verdict KEYING and is silent on coverage — silence is not license to widen. Widening the gate (to cover failed runs that burned spend, or to fail CLOSED on an unresolvable run id under an unattended drain) is a SEPARATE decision, already carried as work-item **`bd-ib-0s5`** (orchestrator tenant, epic `bd-ib-24j5uy`, spec-change-tier, `blocked`/needs-human) and needing its own design record. Do NOT fold it into this accept. A.3's ratified text states the coverage position TIMELESSLY — the two not-gated classes plus the deliberate fail-open disposition — and carries none of this scoping rationale, which has no referent once the change is ratified.
- **Review history.** This proposal was adversarially reviewed by an independent Fable-model reviewer before ratification (the maintainer's standing rule), across three rounds. Round 1 returned TWO blockers: (1) the A.3 cost-gate section asserted three universals the shipped gate falsifies — that every dispatched run is observed, that no verdict is silent, and that the `report` posture still derives the keyed verdict; (2) the lever-precedent citation pointed at `constraints.md` §"Closed-item integrity" instead of the section that actually contracts the lever, `contracts.md` §"Closed-item-integrity check" — a citation that would have resolved mechanically (the heading exists in-tree) and so would have ratified silently. Round 2 returned ONE blocker introduced by the round-1 fix: a gate-coverage RATIONALE paragraph had been nested inside A.3's "It MUST state:" bullet list, so a faithful revise driver would have transcribed change-relative prose ("this proposal", "the design record", "no work-item filed", "retiring `--mode`") into permanent contract text. All three are fixed. Round 1 CLEARED the enforcement-posture lever paragraph as faithful rather than a smuggled relaxation, and round 2 UPHELD the decision to add no `constraints.md` rail for the fail-open gap (a qualified rail would restate the contract rather than constrain it); both verified replacement-target fidelity, the drift sweep, and the criterion-2 grep as clean.
