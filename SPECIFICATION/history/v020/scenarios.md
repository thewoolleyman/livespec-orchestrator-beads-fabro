...
## Scenario 22 — Dispatcher admits the highest-rank ready item up to the per-repo WIP cap

```gherkin
Feature: Dispatcher admission valve with a per-repo WIP cap
  As the Dispatcher enforcing the admission valve
  I want to admit the highest-rank admission-eligible ready item when a slot frees
...
Scenario: A manual-admission item is surfaced rather than admitted
  ...
  And the item has not been explicitly approved into ready by a human
...
Scenario: ai-then-human parks in acceptance until a human confirms
  ...
  And it transitions to done only after a human confirms from the console
