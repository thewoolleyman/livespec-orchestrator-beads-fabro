...
Scenario: Doctor reads spec directly and invokes the thin-transport query skills
  ...
  Then it invokes `/livespec-impl-beads:list-work-items --json` for the work-item structural invariants
...
Scenario: The driver composes both sides' next at the top of each iteration
  ...
  And gap-detection and drift-detection invocations (`/livespec-impl-beads:capture-impl-gaps`, `/livespec-impl-beads:capture-spec-drift`) are likewise Layer 3 driver-side concerns invoked outside `next`'s ranking — `next` ranks materialized work-items only
