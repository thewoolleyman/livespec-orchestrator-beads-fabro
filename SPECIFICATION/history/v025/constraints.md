...
- The Dispatcher's machine-path dispositions of already-filed items
  (the lifecycle verbs admit / complete / accept / reject and the
  `backlog` bounce, with `complete` carrying PR + merge-sha audit
  evidence) are the SOLE exemption from the store-write consent
...
- **Audit every resolution.** Every autonomously-resolved decision MUST be
  journaled and attributable (which valve, which item, what the LLM
  decided); no silent auto-approve — extending the §"Forbidden patterns"
  no-silent-close rule to every collapsed valve.
