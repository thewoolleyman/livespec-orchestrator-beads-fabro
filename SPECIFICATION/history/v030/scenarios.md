...
Feature: orchestrate human valve actions
  ...
  I want approve, accept, reject, set-admission, and set-acceptance commands on the orchestrate run surface
...
Scenario: set-admission edits the policy without touching the status
  ...
  When the operator invokes `orchestrate run --action set-admission:<work-item-id>:auto`
...
## Scenario 33 — Full autonomous mode auto-approves a manual item

```gherkin
Feature: Full autonomous mode collapses the approve gate
  As an operator running unattended under full autonomous mode
  I want manual-admission items auto-approved without a human approval
  So that routine approval decisions do not block an autonomous run

Scenario: A manual-admission item is auto-approved under autonomous mode
  ...
  When the Dispatcher reaches it
  Then it treats the item's effective admission_policy as auto and approves it into `ready`
  And it admits it to `active` through the unchanged mechanical admission valve when a WIP slot frees
  And it journals the auto-approval with the item id and the collapsed gate
```
...
Scenario: A resolvable needs-human item is LLM-resolved and returned to its path
  ...
  And it journals the resolution with the item id and the collapsed gate
```
...
Scenario: Without the explicit opt-in the run stays interactive
  ...
  Then full autonomous mode is not active and the human-delegable gates keep their normal policies
