...
**Human valve actions.** `run` additionally accepts the five human
...
Dispatcher's module docstring documents this boundary. The human-triggered operator commands (`orchestrate run` `approve:`/`accept:`/`reject:`/`set-admission:`/`set-acceptance:` action ids, per §"`orchestrate`") are NOT machine-path dispositions — their consent is the operator's
...
resting at `pending-approval`, through the `orchestrate` human-valve
