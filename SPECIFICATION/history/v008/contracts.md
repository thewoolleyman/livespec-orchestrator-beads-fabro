...
orchestrator-owned: this plugin's four heavyweight SKILL.md
front-ends — `capture-impl-gaps`, `capture-spec-drift`,
`capture-work-item`, `implement` — are exactly those front-ends and
are the governed surface of this discipline.
...
2. `/livespec:doctor` → `/livespec-impl-beads:list-work-items --json`
   (work-item structural invariants).
3. `/livespec:doctor` → `/livespec-impl-beads:detect-impl-gaps --json`
