...
- Heavyweight skills (capture-impl-gaps, capture-spec-drift,
  capture-work-item, implement, groom) carry their orchestration logic
...
- The Dispatcher's machine-path dispositions of already-filed items
  (close-on-confirmed-merge carrying PR + merge-sha audit evidence)
  are the SOLE exemption from the store-write consent discipline;
  `--no-close-on-merge` disables even that path. The Dispatcher MUST
