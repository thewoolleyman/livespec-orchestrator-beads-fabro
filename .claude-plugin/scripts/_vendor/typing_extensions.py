...
#
# Verbatim, byte-faithful copy of the `typing_extensions` module (the
# single-file PyPI distribution), carried source-only inside this
# plugin's `_vendor/` tree so the host-side dispatcher import path
# resolves from the flattened plugin cache with NO `uv sync` and NO
# apt `python3-typing-extensions` — the adopter / fleet-member path.
# Two host-side imports need it:
#   - livespec_runtime.cross_repo.resolve  -> `assert_never`
#   - livespec_orchestrator_beads_fabro.commands._otel_receive -> `override`
# The target interpreter is Python 3.10 (`requires-python >= 3.10.16`);
# `typing.assert_never` (3.11+) and `typing.override` (3.12+) do not
# exist there, so a stdlib swap is NOT possible and the dependency MUST
# be vendored. A `-S` (no-site) regression test pins this invariant:
# tests/bin/test_host_side_self_contained_import.py.
#
# This file is EXCLUDED from this repo's first-party lint / type /
# coverage gates by the `**/_vendor/**` globs (see pyproject.toml).
#
#   Upstream:  https://github.com/python/typing_extensions
#   Version:   4.15.0  (PyPI typing_extensions-4.15.0; git tag 4.15.0)
#   Licence:   PSF-2.0 (Python Software Foundation License, version 2)
#   See NOTICES at the repo root for the attribution entry.
#
# Re-vendor by replacing the body below this header with the
# `typing_extensions.py` from the pinned upstream version and updating
# the `.vendor.jsonc` entry's `vendored_at`.
#
import abc
import builtins
import collections
