...
Mirrors `.claude/hooks/` one-to-one: every non-exempt `.py` hook module there
has its paired `test_<module>.py` here (`check-tests-mirror-pairing`, wired via
the `.claude/hooks` -> `tests/hooks` entry in `pyproject.toml`'s
`mirror_pairings`).

Conventions specific to this tree:

- `.claude/hooks/` is NOT an importable package — Claude Code executes each
