...
_SELF_MERGE_PATHS = (
    ".claude-plugin/scripts/livespec_impl_beads/commands/_dispatcher_engine.py",
    "tests/livespec_impl_beads/commands/test_dispatcher_engine.py",
)
...
def test_wiring_promotes_on_passing_canary_and_does_not_alarm() -> None:
    ...
    _self_update_after_merge(
        ...
        candidate_bin="/pin/candidate/bin/dispatcher.py",
        scratch_root="/tmp/canary-scratch",
        repo=Path("/data/projects/livespec-impl-beads"),
        journal=journal,
        runner=runner,
        poster=poster,
    )
...
def test_parse_pr_files_extracts_repo_relative_paths() -> None:
    stdout = json.dumps(
        {
            "files": [
                {"path": ".claude-plugin/scripts/livespec_impl_beads/commands/dispatcher.py"},
                {"path": "README.md"},
            ]
        }
    )
...
def test_after_verdict_runs_the_gate_for_a_green_pr_self_merge(
    ...
    self_merge_files = json.dumps(
        {"files": [{"path": ".claude-plugin/scripts/livespec_impl_beads/commands/dispatcher.py"}]}
    )
