---
name: loop
description: Layer 3 orchestration driver for livespec-impl-beads. Composes /livespec:next and /livespec-impl-beads:next, dispatches to the appropriate heavyweight skill, runs the project's janitor (typically `just check` plus `/livespec:doctor`) as a hard gate, and loops until the queue drains or the configured budget exhausts. Hand-tuned per repo — local divergence is expected and exempt from copier-update drift detection.
---

# Layer 3 loop driver — livespec-impl-beads starter
...
| `implement` (impl-side) | `/livespec-impl-beads:implement` |
| `capture-impl-gaps` (impl-side) | `/livespec-impl-beads:capture-impl-gaps` |
| `capture-spec-drift` (impl-side) | `/livespec-impl-beads:capture-spec-drift` |
| `process-memos` (impl-side) | `/livespec-impl-beads:process-memos` |
| `none` | exit cleanly |
