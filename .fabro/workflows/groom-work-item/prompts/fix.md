...
first; it is the whole specification of your task. The gate checks four
things and reports exactly one:
...
3. **The draft spans more than one line.** Re-encode it onto ONE line.
   The Dispatcher recovers the draft from a single stderr line, so only
   the first line of a multi-line draft would ever reach the ledger, and
   it would be truncated silently. Use ` ;; ` between slices and ` | `
   between a slice's fields.
4. **The draft carries a brace.** Remove every brace character. A ledger
   comment is append-only, so a comment carrying a template opening
   delimiter would poison every future dispatch of this item
